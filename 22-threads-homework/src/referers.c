...
void putReferer(struct Referers* referers, const char* referer, uint64_t c) {
    ...
    for(struct RefererEntry* curEntry = referers->top; curEntry != NULL ;curEntry = curEntry->next) {
        if (strcmp(referer, curEntry->referer) == 0) {
            curEntry->counter+=c;
            //little sorting to search faster
            if (curEntry->prev != NULL && curEntry->prev->counter < curEntry->counter) {
                //exchange
                struct RefererEntry* pe = curEntry->prev;
                struct RefererEntry* ne = curEntry->next;
                curEntry->next = pe;
                curEntry->prev = pe->prev;
                pe->prev = curEntry;
                pe->next = ne;
            }
            return;
...
void printTopReferers(struct Referers* referers) {
    ...
    for (int i = 0; i < 10 && i < referers->size; i++)
    {
        ...
        printf("%d) Referer = %s, count = %ld\n", i, e->referer, e->counter);
    }
    free(entries);
}
